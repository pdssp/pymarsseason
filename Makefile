...
Usage:\n
	...
	\t\tInstallation\n
	-------------------------------------------------------------------------\n
	make\t\t\t\t                Install pymarsseason in the system (root)\n
	make user\t\t\t 			Install pymarsseason for non-root usage\n
	\n
	-------------------------------------------------------------------------\n
	\t\tDevelopment\n
	-------------------------------------------------------------------------\n
	make prepare-dev\t\t 		Prepare Development environment\n
	make install-dev\t\t 		Install COTS and pymarsseason for development purpose\n
...
prepare-dev:
	cp .pypirc ~${USER} && git init && echo "python3 -m venv pymarsseason-env && export PYTHONPATH=. && export PATH=`pwd`/pymarsseason-env/bin:${PATH}" > ${VENV} && echo "source \"`pwd`/pymarsseason-env/bin/activate\"" >> ${VENV} && scripts/install-hooks.bash && echo "\nnow source this file: \033[31msource ${VENV}\033[0m"
...
lint:  ## Lint and static-check
	flake8 --ignore=E203,E266,E501,W503,F403,F401 --max-line-length=79 --max-complexity=18 --select=B,C,E,F,W,T4,B9 pymarsseason
	pylint pymarsseason
	mypy pymarsseason
...
visu-doc-pdf:
	acroread docs/build/latex/pymarsseason.pdf
